...
$v->addRuleMessage('driver', 'غير صحيح');

$v->addRule('driver', function($value, $input, $args) {
    $value  = trim($value);
    if($args['0'] == 1){
      if($value > 0){
         return (bool) 1;
...
foreach($onumber as $k=>$val){
  ...
  if($by == 'store'){
      $v->validate([
          ...
          'driver'        => [$driverArr[$k],  'driver('.$city_to[$k].')'],
      ...
      $v->validate([
          ...
          'branch_to'     => [$branch_to/*$branch_to[$k]*/,  'required|int'],
          'with_dev'      => [$with_dev,  'required'],
          'driver'        => [$driverArr[$k],  'driver('.$maincity.')'],
          'order_note'    => [$order_note[$k],  'max(250)'],
          'order_address' => [$order_address[$k],  'max(250)'],
          'company' => [$company,  'int'],
      ]);
...
/*            if($_SESSION['user_details']['branch_id'] == $to_branch){
   ...
   foreach($onumber as $k=>$val){
            ...
            $driver = $getdriver[0]['driver_id'];
                    ...
                    $sql = "select * from driver_cities where city_id = ?";
                    $getdriver = getData($con,$sql,[$maincity]);
                    if(count($getdriver) > 0){
                     $driver = $getdriver[0]['driver_id'];
                    }else{
                     $driver = 0;
...
$error = [
           ...
           'driver'=>implode($v->errors()->get('driver')),
