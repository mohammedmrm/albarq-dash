...
try{
  $query = "select companies.*, bar_code from orders left join companies on companies.id = orders.delivery_company_id";
  ...
  foreach($data as $k=>$val){
    $response = httpPost($val['dns'].'api/getOrdersInfo.php',['token'=>$val['token'],"bar_codes"=>[$val['bar_code']]]);
...
echo (json_encode(array($query,$response,"updated"=>$COUNT,"success"=>$success,"data"=>$data)));
?>